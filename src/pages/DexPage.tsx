...
import { generationOptions, isInGeneration } from '../domain/generations';
...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const groups = useMemo(() => {
    ...
    return [{ id: 'national', label: null, items: allSpecies.filter(s => isInGeneration(s.nationalDexNumber, selectedGeneration)).filter(s => matches(s.name, s.nationalDexNumber)).map(species => ({ species, form: forms.find(f => f.id === species.defaultFormId)!, dexNumber: species.nationalDexNumber })) }];
  }, [allSpecies, forms, game, activeSections, hasSectionToggle, selectedSection, selectedGeneration, search, formFor]);
  ...
  const dexSpecies = (activeIds ? activeIds.map(id => allSpecies.find(s => s.id === id)).filter((s): s is Species => !!s) : allSpecies).filter(s => !!game || isInGeneration(s.nationalDexNumber, selectedGeneration));
  ...
  return <><div className="page-head dex-page-head"><div><span className="eyebrow">{game ? 'GAME COLLECTION' : 'CORE COLLECTION'}</span><div className="dex-title-row">{game?.logo && <span className="game-logo"><img src={game.logo} alt={`${game.name} logo`} /></span>}<h1>{game?.name ?? 'National Dex'}</h1></div><p>{shinyOnly ? 'Only shiny collection entries count toward this view.' : 'Thirty slots per box, derived directly from your collection.'}</p></div><div className="page-head-side"><div className="dex-progress-summary"><div><span>Dex completion</span><strong>{progress.obtained} / {progress.total}</strong></div><ProgressBar value={progress.percentage} /><small>{progress.percentage}% complete</small></div><div className="dex-actions"><button onClick={() => addBox(dexItems)}><Plus size={16} /> Complete Dex</button><ConfirmButton className="danger" title="Clear the entire Dex?" description="Every collection entry that contributes to this Dex will be removed, regardless of the active search or filter. This cannot be undone." confirmLabel="Clear Dex" onConfirm={() => clearBox(dexItems)}><Trash2 size={16} /> Clear Dex</ConfirmButton></div></div></div><DexFilters search={search} onSearch={setSearch} ot={otFilter} onOt={setOtFilter} ownership={ownershipFilter} onOwnership={setOwnershipFilter} shinyOnly={shinyOnly} onShiny={setShinyOnly} sections={sectionOptions} selectedSection={selectedSection} onSection={setSelectedSection} generations={!game ? generationOptions : undefined} selectedGeneration={selectedGeneration} onGeneration={setSelectedGeneration} />{boxes.length ? boxes.map(box => <PokemonBox key={box.key} title={box.title} items={box.items} entries={entries} shinyOnly={shinyOnly} ot={otFilter} gameId={gameId} onSelect={f => selectForm(f.id)} onQuickAdd={quickAdd} onAddAll={() => addBox(box.items)} onRemoveAll={() => clearBox(box.items)} />) : <div className="empty">No Pokémon match this search.</div>}{selected && <PokemonDetails form={selected} entries={entries} game={game} onClose={() => selectForm(null)} />}</>;
}
