...
import { filterDexBoxesBySearch, paginateDexGroupList } from '../domain/dexView';
...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const groups = useMemo(() => {
    ...
    if (game && selectedSection === 'all' && hasSectionToggle) {
      ...
      const numberedItems=[...new Map(sectionItems((game.dexSections??[]).filter(section=>section.showDexNumbers!==false)).map(item=>[item.form.id,item])).values()].filter(item=>matchesType(item.form));
  ...
  const boxes = filterDexBoxesBySearch(paginateDexGroupList(visibleGroups, 30, game ? 'game-' : ''),search);
