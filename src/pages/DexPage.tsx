...
import type { AlphaFilter, GameDexSection, OwnershipFilter, PokemonType, ShinyFilter, Species } from '../domain/models';
...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const quickAdd = (form: (typeof forms)[number]) => {
    const relevant = filterEntries(entries,{gameId,shiny,alpha:alphaFilter,ot:otFilter}).filter(e => e.formId === form.id);
  ...
  const groups = useMemo(() => {
    ...
    const matchesType = (form:(typeof forms)[number])=>isInTypes(form.types,selectedTypes);
    if (game && selectedSection === 'all' && hasSectionToggle) {
      const sectionItems = (sections:GameDexSection[])=>sections.flatMap(section=>section.dexSpeciesIds.map((id,index)=>{const species=allSpecies.find(candidate=>candidate.id===id);return species?{species,form:formFor(species,section),dexNumber:index+1,showDexNumber:section.showDexNumbers!==false}:null})).filter((item):item is NonNullable<typeof item>=>item!==null);
    ...
    if (activeSections) return activeSections.map(section => ({ id: section.id, label: section.name, items: section.dexSpeciesIds.map((id, index) => ({ species: allSpecies.find(x => x.id === id), dexNumber: index + 1, showDexNumber:section.showDexNumbers!==false })).filter((x): x is { species: Species; dexNumber: number; showDexNumber:boolean } => !!x.species).filter(x => matches(x.species.name, x.species.nationalDexNumber)).map(x => ({ ...x, form: formFor(x.species, section) })).filter(item=>matchesType(item.form)) })).filter(x => x.items.length);
    return [{ id: 'national', label: null, items: allSpecies.filter(s => isInGenerations(s.nationalDexNumber, selectedGenerations)).filter(s => matches(s.name, s.nationalDexNumber)).map(species => ({ species, form: forms.find(f => f.id === species.defaultFormId)!, dexNumber: species.nationalDexNumber })).filter(item=>matchesType(item.form)) }];
  }, [allSpecies, forms, game, activeSections, hasSectionToggle, selectedSection, selectedGenerations, selectedTypes, search, formFor]);
  const visibleGroups = groups.map(group => ({ ...group, items: group.items.filter(item => { const owned = isFormOwned(entries, item.form.id, { shiny, alpha:alphaFilter, ot: otFilter, gameId }); return ownershipFilter === 'all' || (ownershipFilter === 'owned' ? owned : !owned); }) })).filter(g => g.items.length);
