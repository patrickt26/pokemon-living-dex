...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const clearBox = (items: (typeof boxes)[number]['items']) => { const ids = new Set(items.map(x => x.species.id)); removeMany.mutate(entries.filter(e => ids.has(e.speciesId) && (shiny==='all'||e.shiny===shinyValue) && (alphaFilter==='all'||e.alpha===alphaValue) && (!gameId || e.gameId === gameId)).map(e => e.id)); };
  ...
  const obtained = new Set(entries.filter(e => (!gameId || e.gameId === gameId) && (shiny==='all'||e.shiny===shinyValue) && (alphaFilter==='all'||e.alpha===alphaValue) && dexItems.some(x => x.species.id === e.speciesId && x.form.id === e.formId)).map(e => e.speciesId)).size;
