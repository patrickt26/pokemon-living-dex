...
import type { AlphaFilter, GameDexSection, OwnershipFilter, Species } from '../domain/models';
...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const quickAdd = (form: (typeof forms)[number]) => {
    const relevant = filterEntries(entries,{gameId,shinyOnly,alpha:alphaFilter,ot:otFilter}).filter(e => e.formId === form.id);
    if (hasMultipleOrigins(relevant)) { selectForm(form.id); return; }
    const existing = relevant.find(e => e.gameId === defaultGameId && e.ownOT === (otFilter !== 'other') && e.shiny === shinyOnly && e.alpha === alphaValue) ?? relevant[0];
    if (existing) changeQuantity.mutate({ id: existing.id, quantity: existing.quantity - 1 });
    else add.mutate({ speciesId: form.speciesId, formId: form.id, gameId: defaultGameId, originGameId: defaultGameId, ownOT: otFilter !== 'other', shiny: shinyOnly, alpha:alphaValue, quantity: 1 });
  };
  ...
  const visibleGroups = groups.map(group => ({ ...group, items: group.items.filter(item => { const owned = isFormOwned(entries, item.form.id, { shinyOnly, alpha:alphaFilter, ot: otFilter, gameId }); return ownershipFilter === 'all' || (ownershipFilter === 'owned' ? owned : !owned); }) })).filter(g => g.items.length);
  const boxes = paginateDexGroupList(visibleGroups, 30, game ? 'game-' : '');
  const addBox = (items: (typeof boxes)[number]['items']) => addMany.mutate(items.map(({ species, form }) => ({ speciesId: species.id, formId: form.id, gameId: defaultGameId, originGameId: defaultGameId, ownOT: true, shiny: shinyOnly, alpha:alphaValue, quantity: 1 })));
  const clearBox = (items: (typeof boxes)[number]['items']) => { const ids = new Set(items.map(x => x.species.id)); removeMany.mutate(entries.filter(e => ids.has(e.speciesId) && e.shiny === shinyOnly && (alphaFilter==='all'||e.alpha===alphaValue) && (!gameId || e.gameId === gameId)).map(e => e.id)); };
