...
import type { AlphaFilter, GameDexSection, OwnershipFilter, ShinyFilter, Species } from '../domain/models';
...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const groups = useMemo(() => {
    ...
    if (game && selectedSection === 'all' && hasSectionToggle) {
      const sectionItems = (sections:GameDexSection[])=>sections.flatMap(section=>section.dexSpeciesIds.map((id,index)=>{const species=allSpecies.find(candidate=>candidate.id===id);return species?{species,form:formFor(species,section),dexNumber:index+1,showDexNumber:section.showDexNumbers!==false}:null})).filter(item=>item!==null);
      const numberedItems=[...new Map(sectionItems((game.dexSections??[]).filter(section=>section.showDexNumbers!==false)).map(item=>[item.form.id,item])).values()].filter(item=>matches(item.species.name,item.species.nationalDexNumber));
      const unnumberedGroups=(game.dexSections??[]).filter(section=>section.showDexNumbers===false).map(section=>({id:section.id,label:section.name,items:sectionItems([section]).filter(item=>matches(item.species.name,item.species.nationalDexNumber))}));
      return [{id:'all',label:'All',items:numberedItems},...unnumberedGroups].filter(group=>group.items.length);
    }
    if (activeSections) return activeSections.map(section => ({ id: section.id, label: section.name, items: section.dexSpeciesIds.map((id, index) => ({ species: allSpecies.find(x => x.id === id), dexNumber: index + 1, showDexNumber:section.showDexNumbers!==false })).filter((x): x is { species: Species; dexNumber: number; showDexNumber:boolean } => !!x.species).filter(x => matches(x.species.name, x.species.nationalDexNumber)).map(x => ({ ...x, form: formFor(x.species, section) })) })).filter(x => x.items.length);
  ...
  const gameDexItems = activeSections?.flatMap(section=>section.dexSpeciesIds.map(id=>{const species=allSpecies.find(candidate=>candidate.id===id);return species?{species,form:formFor(species,section),dexNumber:species.nationalDexNumber}:null})).filter(item=>item!==null);
  const dexItems = gameDexItems ? [...new Map(gameDexItems.map(item=>[item.form.id,item])).values()] : allSpecies.filter(species=>isInGenerations(species.nationalDexNumber,selectedGenerations)).map(species=>({species,form:formFor(species),dexNumber:species.nationalDexNumber}));
