...
import { filterEntries, hasMultipleOrigins } from '../domain/collection';
...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  const groups = useMemo(() => {
    ...
    if (game && selectedSection === 'all' && hasSectionToggle) {
      const sectionItems = (sections:GameDexSection[])=>sections.flatMap(section=>section.dexSpeciesIds.map((id,index)=>{const species=allSpecies.find(candidate=>candidate.id===id);return species?{species,form:formFor(species,section),dexNumber:index+1,showDexNumber:section.showDexNumbers!==false,collectBySpecies:collectsBySpecies(species,section)}:null})).filter((item):item is NonNullable<typeof item>=>item!==null);
    ...
    return [{ id: 'national', label: null, items: allSpecies.filter(s => isInGenerations(s.nationalDexNumber, selectedGenerations)).filter(s => matches(s.name, s.nationalDexNumber)).map(species => ({ species, form: forms.find(f => f.id === species.defaultFormId)!, dexNumber: species.nationalDexNumber, collectBySpecies:true })).filter(item=>matchesType(item.form)) }];
  }, [allSpecies, forms, game, activeSections, hasSectionToggle, selectedSection, selectedGenerations, selectedTypes, search, formFor]);
  const ownershipEntries=filterEntries(entries,{shiny,alpha:alphaFilter,ot:otFilter,gameId});
  const visibleGroups = groups.map(group => ({ ...group, items: group.items.filter(item => { const owned=ownershipEntries.some(entry=>(item.collectBySpecies?entry.speciesId===item.species.id:entry.formId===item.form.id)&&entry.quantity>0);return ownershipFilter === 'all' || (ownershipFilter === 'owned' ? owned : !owned); }) })).filter(g => g.items.length);
  ...
  const gameDexItems = activeSections?.flatMap(section=>section.dexSpeciesIds.map(id=>{const species=allSpecies.find(candidate=>candidate.id===id);return species?{species,form:formFor(species,section),dexNumber:species.nationalDexNumber,collectBySpecies:collectsBySpecies(species,section)}:null})).filter((item):item is NonNullable<typeof item>=>item!==null).filter(item=>isInTypes(item.form.types,selectedTypes));
