...
export function DexPage({ gameId }: { gameId?: string }) {
  ...
  return <><div className="page-head dex-page-head"><div><span className="eyebrow">{game ? t('gameCollection','GAME COLLECTION') : t('coreCollection','CORE COLLECTION')}</span><div className="dex-title-row">{game?.logo && <span className="game-logo"><img src={game.logo} alt={`${game.name} logo`} /></span>}<h1>{game?.name ?? t('nationalDex','National Dex')}</h1></div><p>{shiny==='shiny' ? t('shinyDescription','Only shiny collection entries count toward this view.') : t('searchDescription','Thirty slots per box, derived directly from your collection.')}</p></div><div className="page-head-side"><div className="dex-progress-summary"><div><span>{t('dexCompletion','Dex completion')}</span><strong>{progress.obtained} / {progress.total}</strong></div><ProgressBar value={progress.percentage} /><small>{progress.percentage}% {t('completePercent','complete')}</small></div><div className="dex-actions">{game?<button onClick={() => addBox(dexItems)}><Plus size={16} /> {t('complete','Complete Dex')}</button>:<BulkAddGameButton forms={dexItems.map(item=>item.form)} requiresAlpha={alphaFilter==='alpha'} onAdd={(targetGameId,forms)=>addForms(forms,()=>true,targetGameId)}><Plus size={16} /> {t('complete','Complete Dex')}</BulkAddGameButton>}<ConfirmButton className="danger" title={t('clearEntireDex','Clear the entire Dex?')} description={t('clearEntireDexDescription','Every collection entry that contributes to this Dex will be removed, regardless of the active search or filter. This cannot be undone.')} confirmLabel={t('clear','Clear Dex')} onConfirm={() => clearBox(dexItems)}><Trash2 size={16} /> {t('clear','Clear Dex')}</ConfirmButton></div></div></div><DexFilters search={search} onSearch={setSearch} ot={otFilter} onOt={setOtFilter} ownership={ownershipFilter} onOwnership={setOwnershipFilter} shiny={shiny} onShiny={setShiny} alpha={supportsAlpha?alphaFilter:undefined} onAlpha={setSelectedAlpha} sections={sectionOptions} selectedSection={selectedSection} onSection={setSelectedSection} generations={!game ? generationOptions : undefined} selectedGenerations={selectedGenerations} onGeneration={toggleGeneration} selectedTypes={selectedTypes} onType={toggleType} />{boxes.length ? boxes.map(box => <PokemonBox key={box.key} title={box.title} items={box.items} entryIndex={entryIndex} formsById={formsById} shiny={shiny==='shiny'} searchTargetSpeciesId={searchTargetSpeciesId} onSelect={f => selectForm(f.id)} onQuickAdd={quickAdd} onAddAll={game?() => addBox(box.items):undefined} addAllButton={!game?<BulkAddGameButton forms={box.items.map(item=>item.form)} requiresAlpha={alphaFilter==='alpha'} buttonTitle={t('addAllDescription','Add one of each Pokémon in this box')} onAdd={(targetGameId,forms)=>addForms(forms,()=>true,targetGameId)}><Plus size={14}/> {t('addAll','Add all')}</BulkAddGameButton>:undefined} onRemoveAll={() => clearBox(box.items)} />) : <div className="empty">{t('searchEmpty','No Pokémon match this search.')}</div>}{selected && <PokemonDetails form={selected} entries={entries} game={game} onClose={() => selectForm(null)} />}</>;
}
