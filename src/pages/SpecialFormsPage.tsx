...
import type { AlphaFilter, OtFilter, OwnershipFilter, PokemonForm } from '../domain/models';
...
export function SpecialFormsPage(){
  ...
  const obtained=new Set(entries.filter(entry=>entry.quantity>0&&groupFormIds.has(entry.formId)&&entry.shiny===shinyOnly&&(alpha==='all'||entry.alpha===alphaValue)).map(entry=>entry.formId)).size;
  ...
  const addForms=(items:{form:PokemonForm}[])=>addMany.mutate(items.filter(item=>alpha!=='alpha'||eligible(item.form)).map(({form})=>({speciesId:form.speciesId,formId:form.id,gameId:'home',originGameId:'home',ownOT:true,shiny:shinyOnly,alpha:alphaValue,quantity:1})));
  const clearForms=(items:{form:PokemonForm}[])=>{const ids=new Set(items.map(item=>item.form.id));removeMany.mutate(entries.filter(entry=>ids.has(entry.formId)&&entry.shiny===shinyOnly&&(alpha==='all'||entry.alpha===alphaValue)).map(entry=>entry.id))};
