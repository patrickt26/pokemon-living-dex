...
import type { AlphaFilter, OtFilter, OwnershipFilter, PokemonForm, Region } from '../domain/models';
...
export function RegionalPage(){
  ...
  const quickAdd=(form:PokemonForm)=>{
    const relevant=filterEntries(entries,{shinyOnly,alpha,ot}).filter(entry=>entry.formId===form.id);
    if(shouldOpenEntryPicker(relevant)){selectForm(form.id);return}
    const existing=relevant.find(entry=>entry.gameId==='home'&&entry.ownOT===(ot!=='other')&&entry.shiny===shinyOnly&&entry.alpha===alphaValue)??relevant[0];
    if(existing)changeQuantity.mutate({id:existing.id,quantity:existing.quantity-1});
    else add.mutate({speciesId:form.speciesId,formId:form.id,gameId:'home',originGameId:'home',ownOT:ot!=='other',shiny:shinyOnly,alpha:alphaValue,quantity:1});
  };
  const addBox=(items:{form:PokemonForm}[])=>addMany.mutate(items.filter(item=>alpha!=='alpha'||eligible(item.form)).map(({form})=>({speciesId:form.speciesId,formId:form.id,gameId:'home',originGameId:'home',ownOT:true,shiny:shinyOnly,alpha:alphaValue,quantity:1})));
  const clearBox=(items:{form:PokemonForm}[])=>{const ids=new Set(items.map(item=>item.form.id));removeMany.mutate(entries.filter(entry=>ids.has(entry.formId)&&entry.shiny===shinyOnly&&(alpha==='all'||entry.alpha===alphaValue)).map(entry=>entry.id))};
  ...
  const obtained=new Set(entries.filter(entry=>entry.quantity>0&&formIds.has(entry.formId)&&entry.shiny===shinyOnly&&(alpha==='all'||entry.alpha===alphaValue)).map(entry=>entry.speciesId)).size;
