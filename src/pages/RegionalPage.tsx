...
export function RegionalPage(){
  ...
  return <><div className="page-head dex-page-head"><div><span className="eyebrow">{t('regionalCollection')}</span><h1>{t('regionalForms','Regional Forms')}</h1><p>{shiny==='shiny'?t('regionalShinyDescription','Only shiny regional entries count toward this view.'):t('regionalDescription','Regional forms remain connected to their National Dex species.')}</p></div><div className="page-head-side"><div className="dex-progress-summary"><div><span>{t('dexCompletion','Dex completion')}</span><strong>{obtained} / {speciesIds.length}</strong></div><ProgressBar value={percentage}/><small>{percentage}% {t('completePercent')}</small></div><div className="dex-actions"><BulkAddGameButton forms={regionalItems.map(item=>item.form)} requiresAlpha={alpha==='alpha'} onAdd={(targetGameId,forms)=>addForms(forms,form=>alpha!=='alpha'||eligible(form),targetGameId)}><Plus size={16}/> {t('complete','Complete Dex')}</BulkAddGameButton><ConfirmButton className="danger" title={t('clearRegional','Clear all regional forms?')} description={t('clearRegionalDescription','Every regional form entry will be removed from your collection. This cannot be undone.')} confirmLabel={t('clear','Clear Dex')} onConfirm={()=>clearBox(regionalItems)}><Trash2 size={16}/> {t('clear','Clear Dex')}</ConfirmButton></div></div></div><DexFilters search={search} onSearch={setSearch} shiny={shiny} onShiny={setShiny} alpha={alpha} onAlpha={setAlpha} ownership={ownership} onOwnership={setOwnership} ot={ot} onOt={setOt} selectedTypes={selectedTypes} onType={toggleType}/>{displayedGroups.map(({region,items})=><PokemonBox key={region} title={labels[region]} items={items} entryIndex={entryIndex} formsById={formsById} shiny={shinyValue} searchTargetSpeciesId={searchTargetSpeciesId} onSelect={form=>selectForm(form.id)} onQuickAdd={quickToggle} addAllButton={<BulkAddGameButton forms={items.map(item=>item.form)} requiresAlpha={alpha==='alpha'} buttonTitle={t('addAllDescription','Add one of each Pokémon in this box')} onAdd={(targetGameId,forms)=>addForms(forms,form=>alpha!=='alpha'||eligible(form),targetGameId)}><Plus size={14}/> {t('addAll','Add all')}</BulkAddGameButton>} onRemoveAll={()=>clearBox(items)}/>) }{selected&&<PokemonDetails form={selected} entries={entries} onClose={()=>selectForm(null)}/>}</>;
}
