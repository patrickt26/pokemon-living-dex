...
class MemoryRepository implements CollectionRepository{
  ...
  async addEntry(input:CollectionEntryInput){const entry={...input,id:String(this.entries.length),createdAt:'x',updatedAt:'x'};this.entries.push(entry);return entry}
  ...
  async replaceEntries(inputs:CollectionEntryInput[]){this.entries=inputs.map((input,index)=>({...input,id:String(index),createdAt:'x',updatedAt:'x'}))}
}
const input:CollectionEntryInput={speciesId:'species-25',formId:'form-25-default',gameId:'home',originGameId:'home',shiny:false,ownOT:true,quantity:2};
describe('CollectionBackupService',()=>{
  ...
  it('exports collection entries as escaped CSV',async()=>{const service=new CollectionBackupService(new MemoryRepository([{...input,id:'1',createdAt:'x',updatedAt:'x'}]),pokemonDataSource);const csv=await service.exportCsv();expect(csv).toContain('"National Dex","Pokémon","Form"');expect(csv).toContain('"25","Pikachu"');expect(csv.split('\n')).toHaveLength(2)});
  it('reports invalid local collection entries',async()=>{const invalid={...input,id:'bad',formId:'form-26-default',quantity:0,createdAt:'x',updatedAt:'x'};const service=new CollectionBackupService(new MemoryRepository([invalid]),pokemonDataSource);const issues=await service.validateCollection();expect(issues.map(issue=>issue.message)).toEqual(expect.arrayContaining(['Form does not belong to the species.','Quantity must be a positive integer.']))});
  ...
  it('merges matching entries by summing their quantities',async()=>{const repository=new MemoryRepository([{...input,id:'old',createdAt:'x',updatedAt:'x'}]);const service=new CollectionBackupService(repository,pokemonDataSource);await service.importCollection({version:1,exportedAt:'2026-01-01',entries:[{...input,quantity:3}]},'merge');expect(repository.entries).toHaveLength(1);expect(repository.entries[0]?.quantity).toBe(5)});
  it('replaces the existing collection',async()=>{const repository=new MemoryRepository([{...input,id:'old',createdAt:'x',updatedAt:'x'}]);const service=new CollectionBackupService(repository,pokemonDataSource);await service.importCollection({version:1,exportedAt:'2026-01-01',entries:[{...input,speciesId:'species-26',formId:'form-26-default'}]},'replace');expect(repository.entries).toHaveLength(1);expect(repository.entries[0]?.speciesId).toBe('species-26')});
});
