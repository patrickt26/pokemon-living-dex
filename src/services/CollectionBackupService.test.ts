...
describe('CollectionBackupService',()=>{
  ...
  it('normalizes former standard variant IDs from older backups',()=>{const service=new CollectionBackupService(new MemoryRepository(),pokemonDataSource);const preview=service.parse(JSON.stringify({version:1,exportedAt:'2026-01-01',entries:[{...input,speciesId:'species-201',formId:'form-201-a'}]}));expect(preview.backup.entries[0]?.formId).toBe('form-201-default')});
