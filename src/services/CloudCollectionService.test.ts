...
function setup(){
  ...
  const rpc=vi.fn().mockImplementation((name:string)=>Promise.resolve(name==='get_collection_summary'?{data:[{entries:1,species:1,total_quantity:2}],error:null}:name==='get_collection_snapshot'?{data:[{species_id:'species-25',form_id:'form-25',game_id:'sv',origin_game_id:'home',shiny:true,alpha:false,own_ot:true,quantity:2}],error:null}:{data:null,error:null}));
