...
export class CollectionService {
  ...
  add(input:CollectionEntryInput){if(input.quantity<1) throw new Error('Quantity must be at least 1'); return this.repository.addEntry({...input,originGameId:input.originGameId??input.gameId});}
  ...
  async changeOne(id:string,changes:Partial<CollectionEntryInput>){
    ...
    const changed={...source,...changes}; if(changed.ownOT===source.ownOT&&changed.shiny===source.shiny&&changed.gameId===source.gameId&&changed.originGameId===source.originGameId&&changed.formId===source.formId)return;
    const target=entries.find(entry=>entry.id!==source.id&&entry.speciesId===changed.speciesId&&entry.formId===changed.formId&&entry.gameId===changed.gameId&&entry.originGameId===changed.originGameId&&entry.shiny===changed.shiny&&entry.ownOT===changed.ownOT);
    ...
    await this.repository.updateEntry(source.id,{quantity:source.quantity-1}); if(target)await this.repository.updateEntry(target.id,{quantity:target.quantity+1});else await this.repository.addEntry({speciesId:changed.speciesId,formId:changed.formId,gameId:changed.gameId,originGameId:changed.originGameId,shiny:changed.shiny,ownOT:changed.ownOT,quantity:1});
  }
  async addOrIncrementMany(inputs:CollectionEntryInput[]){const entries=await this.repository.getEntries();for(const raw of inputs){const input={...raw,originGameId:raw.originGameId??raw.gameId};const existing=entries.find(entry=>entry.speciesId===input.speciesId&&entry.formId===input.formId&&entry.gameId===input.gameId&&(entry.originGameId??entry.gameId)===input.originGameId&&entry.shiny===input.shiny&&entry.ownOT===input.ownOT);if(existing){existing.quantity+=input.quantity;await this.repository.updateEntry(existing.id,{quantity:existing.quantity})}else{const created=await this.repository.addEntry(input);entries.push(created)}}}
