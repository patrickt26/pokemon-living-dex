...
export class CollectionService {
  ...
  add(input:CollectionEntryInput){if(input.quantity<1) throw new Error('Quantity must be at least 1'); return this.repository.addEntry({...input,formId:normalizeVariantFormId(input.formId),originGameId:input.originGameId??input.gameId,alpha:input.alpha??false});}
  ...
  async changeOne(id:string,changes:Partial<CollectionEntryInput>){
    ...
    if(source.quantity===1){if(target){await this.repository.updateEntry(target.id,{quantity:target.quantity+1});await this.repository.removeEntry(source.id)}else await this.repository.updateEntry(source.id,normalizedChanges);return}
