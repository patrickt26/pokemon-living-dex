...
export function PokemonDetails({ form, entries, game, onClose }: { form:PokemonForm; entries:CollectionEntry[]; game?:Game; onClose:()=>void }) {
  ...
  const addOne = () => add.mutate({ speciesId:species.id, formId:selectedForm, gameId, ownOT, shiny, alpha:showAlpha&&alpha, quantity:1 });
  ...
  return <div className="drawer-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><aside ref={panelRef} className="details" role="dialog" aria-modal="true" aria-labelledby="pokemon-details-title" tabIndex={-1}>
    ...
    <div className="details-hero"><PokemonSprite src={shiny ? (form.shinySprite ?? form.sprite) : form.sprite} name={species.name}/><div className="details-heading"><div className="dex-labels"><span className="dex-label-current">{t('nationalDex','National Dex')} #{String(species.nationalDexNumber).padStart(4, '0')}</span>{localDexNumbers.map(label => <span className="dex-label-national" key={label}>{label}</span>)}</div><h2 id="pokemon-details-title">{species.name}</h2><p>{form.name} {t('formLower','form')}</p></div></div>
    <div className="stat-row"><div><strong>{summary.total}</strong><span>{t('total','Total')}</span></div><div><strong>{summary.ownOT}</strong><span>{t('ownOt','Own OT')}</span></div><div><strong>{summary.otherOT}</strong><span>{t('otherOtStat','Other OT')}</span></div><div><strong>{summary.shiny}</strong><span>Shiny</span></div></div>
