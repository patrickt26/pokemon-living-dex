...
export function PokemonDetails({ form, entries, game, onClose }: { form:PokemonForm; entries:CollectionEntry[]; game?:Game; onClose:()=>void }) {
  ...
  return <div className="details-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><aside ref={panelRef} className="details" role="dialog" aria-modal="true" aria-labelledby="pokemon-details-title" tabIndex={-1}>
    ...
    <div className="stat-row"><div><strong>{summary.total}</strong><span>{t('total','Total')}</span></div><div><strong>{summary.ownOT}</strong><span>{t('ownOt','Own OT')}</span></div><div><strong>{summary.otherOT}</strong><span>{t('otherOtStat','Other OT')}</span></div><div><strong>{summary.shiny}</strong><span>{t('shinyMode','Shiny')}</span></div></div></div>
    <EvolutionChain speciesId={species.id}/>
    <div className="details-actions-grid"><section><h3>{t('byGame','By game')}</h3><div className="badges">{availableGames.map(candidate => <GameBadge key={candidate.id} game={candidate} count={summary.byGame[candidate.id] ?? 0}/>)}</div></section>
    <section><h3>{t('quickAdd','Quick add')}</h3><div className="quick-add-fields"><label>{t('form','Form')}<select value={selectedForm} onChange={event => setSelectedForm(event.target.value)}>{forms.map(candidate => <option value={candidate.id} key={candidate.id}>{candidate.name}</option>)}</select></label><label>{t('game','Game')}<select value={gameId} onChange={event => setGameId(event.target.value)} disabled={!availableGames.length}>{!availableGames.length && <option value="home">{t('noCompatibleGame','No compatible game')}</option>}{availableGames.map(candidate => <option value={candidate.id} key={candidate.id}>{candidate.name}</option>)}</select></label></div><div className="toggle-row"><button className={ownOT ? 'active' : ''} onClick={() => setOwnOT(value => !value)}>{t('ownOt','Own OT')}</button><button className={shiny ? 'active' : ''} onClick={() => setShiny(value => !value)}>{t('shiny','✨ Shiny')}</button>{showAlpha&&<button className={alpha ? 'active' : ''} onClick={() => setAlpha(value => !value)}><AlphaIcon /> {t('alpha','Alpha')}</button>}</div><button className="primary" onClick={addOne} disabled={!availableGames.length}><Plus size={18}/> {t('addOne','Add one')}</button></section></div>
