...
interface Props { species: Species; form: PokemonForm; owned: boolean; quantity: number; alpha?: boolean; shiny?: boolean; dexNumber?: number; onClick: () => void; onQuickAdd?: () => void }
export const PokemonSlot = memo(function PokemonSlot({ species, form, owned, quantity, alpha=false, shiny=false, dexNumber = species.nationalDexNumber, onClick, onQuickAdd }: Props) {
  ...
  return <button className={`pokemon-slot ${owned ? 'owned' : 'missing'}`} onClick={handleClick} onDoubleClick={handleDoubleClick} onKeyDown={handleKeyDown} aria-label={`${species.name}, National Dex #${dexNumber}, ${quantity > 0 ? `${quantity} owned` : 'missing'}`} title="Click for details · Double-click or Space to add or remove one"><span className="dex-number">#{String(dexNumber).padStart(3, '0')}</span>{shiny&&<span className="shiny-tag" aria-label="Shiny" title="Shiny">✨</span>}{alpha&&<span className={`alpha-tag ${shiny?'below-shiny':''}`} aria-label="Alpha" title="Alpha"><AlphaIcon size={17}/></span>}{quantity > 0 && <span className="quantity">×{quantity}</span>}<PokemonSprite src={form.sprite} name={species.name} dim={!owned} /><strong>{species.name}</strong>{form.name !== 'Standard' && <small>{form.name}</small>}</button>;
});
