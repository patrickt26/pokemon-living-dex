...
export const PokemonSlot = memo(function PokemonSlot({ species, form, owned, quantity, alpha=false, shiny=false, highlighted=false, dexNumber = species.nationalDexNumber, showDexNumber=true, onClick, onQuickAdd }: Props) {
  ...
  return <button className={`pokemon-slot ${owned ? 'owned' : 'missing'}${highlighted?' search-match':''}`} onClick={handleClick} onDoubleClick={handleDoubleClick} onKeyDown={handleKeyDown} aria-label={`${species.name}, ${showDexNumber?`${t('nationalDex')} #${dexNumber}, `:''}${quantity > 0 ? `${quantity} ${t('slotOwned')}` : t('slotMissing')}`} title={onQuickAdd?t('slotHint'):t('slotDetailsHint')}>{showDexNumber&&<span className="dex-number">#{String(dexNumber).padStart(3, '0')}</span>}{shiny&&<span className="shiny-tag" aria-label={t('shinyMode')} title={t('shinyMode')}>✨</span>}{alpha&&<span className={`alpha-tag ${shiny?'below-shiny':''}`} aria-label={t('alpha')} title={t('alpha')}><AlphaIcon size={17}/></span>}{quantity > 0 && <span className="quantity">×{quantity}</span>}<PokemonSprite src={form.sprite} name={species.name} dim={!owned} /><strong>{species.name}</strong><span className="pokemon-slot-types" aria-hidden="true">{form.types.map(type=><TypeIcon type={type} key={type}/>)}</span>{form.name !== 'Standard' && <small>{form.name}</small>}</button>;
});
