...
export class DexieCollectionRepository implements CollectionRepository {
  ...
  async addEntry(input:CollectionEntryInput){ const now=new Date().toISOString(); const entry:CollectionEntry={...input,alpha:input.alpha??false,id:crypto.randomUUID(),createdAt:now,updatedAt:now}; await this.database.entries.add(entry); return entry; }
  ...
  async replaceEntries(inputs:CollectionEntryInput[]){
    ...
    const entries=inputs.map(input=>({...input,alpha:input.alpha??false,id:crypto.randomUUID(),createdAt:now,updatedAt:now}));
