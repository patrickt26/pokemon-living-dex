...
export function isInGenerations(nationalDexNumber:number,generations:string[]){return generations.length===0||generations.some(generation=>isInGeneration(nationalDexNumber,generation))}
export function toggleGenerationSelection(selected:string[],generation:string){if(generation==='all')return [];return selected.includes(generation)?selected.filter(value=>value!==generation):[...selected,generation]}
